...
![output](screenshots/git-status-ls.png)
...
![terminal output](screenshots/terminal-installed-output.png)
...
![extension](screenshots/ans-ext1.png)
![extension](screenshots/ans-ext2.png)
...
![editor opened](screenshots/vscode-content-open.png)
...
![ansible.cfg](screenshots/ans-cfg-config.png)
...
![terminal output](screenshots/term-output.png)
...
![loaded key](screenshots/key-loaded.png)
...
![git details](screenshots/git-details.png)
...
![all files ](screenshots/all-files.png)
...
![ansible version and other details](screenshots/ans-vers.png)
...
![Ansible onboarding](final-ansible-onboarding.png)
---
...
The combination of .editorconfig and pinned hook versions in .pre-commit-config.yaml. It automatically enforces uniform indentation (2 spaces), line endings (LF), and exact linter versions across the entire team, preventing formatting debates and syntax breakages before code is committed.

---

**2. What is one pitfall you avoided while completing the setup?**

Installing Ansible globally using system-level pip or Ubuntu apt. Isolating packages inside .venv protected the underlying operating system Python packages from corruption and ensured reproducible tooling via requirements.txt.

---

**3. Why should Ansible be installed inside a Python virtual environment?**

Ansible and its companion tools (ansible-lint, yamllint) require specific Python libraries and runtime versions. A virtual environment isolates these dependencies from system Python packages and other projects, avoiding dependency version conflicts.

---

**4. Why must SSH private keys and `.venv/` remain outside version control?**

SSH private keys provide unrestricted administrative access to remote infrastructure; exposing them compromises security. The .venv/ directory contains system-specific binaries, machine symlinks, and dependencies that are non-portable across systems and add unnecessary bloat to version control.
